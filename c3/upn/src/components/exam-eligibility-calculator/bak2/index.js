...
const ExamEligibilityCalculator = () =>{
 ...
 useEffect(()=>{ // setDefault Values Dynamically into formData
  let initialFormData = {};
  let listElements = {};
  EligibilityData?.data?.[activeMenuId]?.list?.map((l,i2)=>{
    listElements[l.id] = { value: l?.defaultValue, validationSuccess:[], errorMessage:'' };
  });
  initialFormData[menuList[activeMenuId]] = listElements;
