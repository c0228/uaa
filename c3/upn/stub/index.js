...
const transformCache = (data) => {
  ...
  Object.entries(data.cacheData.niches).forEach(([category, subs]) => {
    const categorySlug = toSlug(category);

    result.cacheData.niches[categorySlug] = {
      label: category,
      subcategories: {}
    };
...
const output = transformCache(input);
...
// write to file
fs.writeFileSync("cache-output.json", jsonData, "utf-8");

console.log("File saved: cache-output.json");